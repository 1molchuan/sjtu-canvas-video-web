...
Phase 3 增加 React 产品界面、Axum 同源静态资源和可复现发布包。当前私人生产实例运行在 Mac mini，通过 Cloudflare Tunnel 暴露 `canvas-sjtu.1molchuan.top`；Axum 仍只监听 loopback。Ubuntu + systemd + Caddy 文件作为可选部署路线保留。本地 `206` 不能替代 Cloudflare 下的 Range 与不缓存验证。
...
也可以启用一次性邀请链接，让朋友用自己的 jAccount 扫码后自动加入动态白名单。服务端只持久化邀请令牌和稳定身份的哈希；管理与安全边界见 [docs/invitations.md](docs/invitations.md)。
...
当前 Mac mini 拓扑为 `Cloudflare Tunnel → http://127.0.0.1:3100`，见 [docs/deployment-macos-cloudflare.md](docs/deployment-macos-cloudflare.md)。Ubuntu 可选拓扑为 `Cloudflare → Caddy → http://127.0.0.1:3100`；Caddy 不提供静态文件、不缓存、不压缩且不开 access log。相关文档：
...
中国内地云服务器必须先完成 ICP 备案及当前接入商的接入备案。安全组放行 80/443 不能替代备案；未备案时不得把本机 HTTPS 成功误记为公网成功。当前部署保留 Ubuntu release、systemd 和 Caddy，但没有公开 DNS 解析。

## 安全不变量

- 每个网站 Session 独占 `ProtocolContext` 与 Cookie Jar；不存在全局上游 Cookie 或课程 token。
- 默认代理模式下，上游 Cookie、`tokenId`、视频 token、真实资源 ID 和视频 URL不进入浏览器 JSON或持久化存储。显式实验直连会通过 `307` 向浏览器暴露短期视频 URL，默认关闭，见 `docs/direct-download-experiment.md`。
- Stable ID 只用于服务端白名单；界面不显示原文或可离线比对的哈希。
- 一次性邀请令牌位于 URL fragment，浏览器读取后立即清除；数据库只保存令牌哈希和动态白名单身份哈希。
...
- 动态邀请白名单保存在私有 SQLite 中；撤销影响后续登录，不会强制终止已经存在的网站 Session。
- 视频只支持单轨、单 Range 下载；支持将录像系统已有的识别结果下载为 SRT，但不支持多 Range、批量下载、在线播放、PPT、转码或合并。
