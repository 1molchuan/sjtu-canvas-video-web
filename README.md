...
2026-07-17 已将精确 release 部署到 Ubuntu，systemd、Caddy、loopback HTTPS、静态/API 路由、重启恢复和服务器安全审计均已通过。该服务器位于阿里云中国内地，未备案域名的 80/443 流量在到达虚拟机前被拦截；`canvas.1molchuan.top` 的 DNS 记录已按维护者要求移除，因此公网登录和下载验收当前为 `blocked`，不能视为生产公网完成。
...
- 当前阿里云中国内地 origin 因 ICP 备案条件未满足而无法接收域名公网流量；DNS 已移除，公网验收为 `blocked`。
