...
The production config can enable the private invitation database:

```toml
[invites]
database_path = "/var/lib/canvas-video/invites.sqlite3"
default_ttl_hours = 24
```

The systemd unit uses `StateDirectory=canvas-video` for a service-private state directory. The invitation database is outside every immutable release; see [invitations.md](invitations.md) for management commands and the data boundary.
