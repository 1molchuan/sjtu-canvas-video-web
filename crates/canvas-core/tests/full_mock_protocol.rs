...
use canvas_core::{
    ...
    video::{get_subtitle, get_video_info, list_course_videos_with_refresh, probe_video_track},
};
use tokio::sync::mpsc;
...
async fn complete_mock_protocol_chain_reuses_one_isolated_context() {
    ...
    let subtitle = get_subtitle(&context, &catalog.auth, detail.source_course_id)
        .await
        .expect("subtitle should succeed");
    ...
    assert!(subtitle.srt.contains("第一句"));
}
